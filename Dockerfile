...
RUN sudo vim /etc/php5/fpm/php.ini +385 -c 'd' -c 'i' -c ':r! echo "max_execution_time = 60"' -c ':wq'
RUN sudo vim /etc/memcached.conf +23 -c 'd' -c 'i' -c ':r! echo "-m 3074"' -c ':wq'
CMD service php5-fpm start && nginx && memcached
