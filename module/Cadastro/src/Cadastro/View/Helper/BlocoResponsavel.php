...
class BlocoResponsavel extends AbstractHelper {
    ...
    public function renderHtml() {
        ...
        $funcaoJS = str_replace('#tipo', $this->getTipo(), ConstantesCadastro::$FUNCAO_JS_ABRIR_TELAS_DE_ALUNO);
        ...
        switch ($this->getTipo()) {
            case 1:
                $textoInsiraOResponsavel = $this->view->translate(ConstantesCadastro::$TRADUCAO_INSIRA_OS_DADOS . ConstantesCadastro::$TRADUCAO_HOMEM);
                break;
            case 2:
                $textoInsiraOResponsavel = $this->view->translate(ConstantesCadastro::$TRADUCAO_INSIRA_OS_DADOS . ConstantesCadastro::$TRADUCAO_MULHER);
                break;
            default:
                $textoInsiraOResponsavel = $this->view->translate(ConstantesCadastro::$TRADUCAO_INSIRA_OS_DADOS . ConstantesCadastro::$TRADUCAO_RESPONSAVEL);
                break;
        }
        ...
        $conteudo[$contadoDeDiv] = $this->view->botaoLink(
                ConstantesCadastro::$TRADUCAO_INSERIR, ConstantesForm::$STRING_HASHTAG, 5, $this->view->funcaoOnClick('abrirTelaDeAlunos(' . $this->getTipo() . ')'));
        ...
        $class[$contadoDeDiv] = ConstantesForm::$FORM_HIDDEN;
        $conteudo[$contadoDeDiv] = $this->view->botaoLink(
                ConstantesCadastro::$TRADUCAO_LIMPAR, ConstantesForm::$STRING_HASHTAG, 6, $this->view->funcaoOnClick('limparDadosPessoaSelecionada(' . $this->getTipo() . ')'));
        $html .= $this->view->blocoDiv($id[$contadoDeDiv], $class[$contadoDeDiv], $conteudo[$contadoDeDiv]);
