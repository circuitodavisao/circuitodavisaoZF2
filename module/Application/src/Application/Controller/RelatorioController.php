...
public function alunosAction(){
	...
	$listaDeAulasAtivas = array();
	foreach($turmas as $turma){
		if($turma->getTurmaAulaAtiva()){						
			$listaDeAulasAtivas[$turma->getId()] = $turma->getTurmaAulaAtiva();
		}else{
			$listaDeAulasAtivas[$turma->getId()] = null;
		}
	}
	...
	foreach($relatorioInicial as $relatorio){
		if(
				...
				$relatorio->getSituacao_id() === Situacao::ESPECIAL
			&& $listaDeAulasAtivas[$relatorio->getTurma_id()] !== null
		){
			...
			if($turmaPessoa->verificarSeEstaAtivo()){
				...
				if($tipoRelatorio === self::relatorioAlunosQueNaoForamAAula){
					...
					if(!$assistiuAAula){
						...
						$relatorioAjustado[$relatorio->getTurma_id()][] = $dados;
					}
				}
				...
				if($tipoRelatorio === self::relatorioAlunosComFaltas){
					...
					$aulaAtiva = $listaDeAulasAtivas[$relatorio->getTurma_id()];
					if($aulaAtiva){
						foreach ($aulaAtiva->getAula()->getDisciplina()->getAulaOrdenadasPorPosicao() as $aula) {
							$temFalta = true;
							if($aulaAtiva->getAula()->getId() === $aula->getId()){
								break;
							}
				...
				if(
					...
					if(!in_array($idGrupo, $listaDeEquipes)){
						$nomeEquipe = $grupo->getGrupoEquipe()->getEntidadeAtiva()->getNome();
						if($nomeEquipe == ''){
							$nomeEquipe = $grupo->getGrupoIgreja()->getEntidadeAtiva()->getNome();
						}
						$listaDeEquipes[$idGrupo] = $nomeEquipe;
					}else{
						$nomeEquipe = $listaDeEquipes[$idGrupo];
					...
					$dados = null;
					if($relatorioAjustado[$relatorio->getTurma_id()][$nomeEquipe]){
						$dados = $relatorioAjustado[$relatorio->getTurma_id()][$nomeEquipe];
					}else{
						$dados = array();
						$dados['time'] = $nomeEquipe;
						$dados['assitiu'] = 0;
					...
					$relatorioAjustado[$relatorio->getTurma_id()][$nomeEquipe] = $dados;
				}
			}
		}
	}
										...
										foreach($grupoPaiFilhoFilhos1728 as $grupoFilho1728){
											...
											if($filho1728->getEntidadeAtiva()){
												...
												if($buscarAbaixo20736){
													...
													if($buscarAbaixo248832){
														$grupoPaiFilhoFilhos248832 = $filho20736->getGrupoPaiFilhoFilhosAtivos($periodoParaUsar);
														foreach($grupoPaiFilhoFilhos248832 as $grupoFilho248832){
															$filho248832 = $grupoFilho248832->getGrupoPaiFilhoFilho();								

															if($filho248832->getEntidadeAtiva()){
																$buscarAbaixo2985984 = false;
																if($filho248832->getEntidadeAtiva()->getEntidadeTipo()->getId() === EntidadeTipo::regiao){
																	$regioes++;
																	$buscarAbaixo2985984 = true;
																}
																if($filho248832->getEntidadeAtiva()->getEntidadeTipo()->getId() === EntidadeTipo::coordenacao){
																	$coordenacoes++;
																	$buscarAbaixo2985984 = true;
																}
																if($filho248832->getEntidadeAtiva()->getEntidadeTipo()->getId() === EntidadeTipo::igreja){
																	$igrejas++;
																	$numeroIdentificador248832 = $repositorio->getFatoCicloORM()->montarNumeroIdentificador($repositorio, $filho248832);
																	$fatoMensalSomado = $repositorio->getFatoMensalORM()->buscarFatosSomadosPorNumeroIdentificadorMesEAno($numeroIdentificador248832, date('m'), date('Y'), $somado = 2);			
																	/* Somando líderes */
																	$l = 'l'.$contadorDePeriodos;
																	$lideres += $fatoMensalSomado[$l];
																	/* Somando Células */
																	...
																	$celulas += $fatoMensalSomado[$cq] + $fatoMensalSomado[$cbq];
																	/* Somando discipulados */
																	$discipulados += $repositorio->getFatoCelulaDiscipuladoORM()->totalAtivosPorNumeroIdentificador($numeroIdentificador248832);
																	/* Somando alunos */
																	$alunos += RelatorioController::totalDeAlunos($repositorio, $filho248832);
																	/* Somado parceiro de Deus */
																	$parceiro += $repositorio->getFatoFinanceiroORM()->fatosValorPorNumeroIdentificadorMesEAno($numeroIdentificador248832, $mes, $ano)['valor'];					
																}
															}
														}
													}
...
//		$listaDeGrupos[] = 4897;
	...
	public static function relatorioCompletoNovo($repositorio, $grupo, $tipoRelatorio, $mes, $ano, $tudo = true, $pessoalOuEquipe = 2, $periodo = 0, $relatorioDoLider = 1) {
		...
		if($grupo->getEntidadeAtiva()->getEntidadeTipo()->getId() !== EntidadeTipo::regiao
			...
			&& $grupo->getEntidadeAtiva()->getEntidadeTipo()->getId() !== EntidadeTipo::presidencial ){
				...
				for($w = 1;$w <= 6;$w++){
					...
					$e = 'e' . $w;
					$ep = 'ep' . $w;
					$fatoFilho->$ep = ($fatoFilho->$e / $quantidadeDeCelulas * 100);
				}
				...
				$somaEp = $fatoFilho->ep1 + $fatoFilho->ep2 + $fatoFilho->ep3 + $fatoFilho->ep4 + $fatoFilho->ep5 + $fatoFilho->ep6;
				...
				$fatoFilho->mediaElitePerformance = ($somaEp/$diferencaDePeriodos);
				...
				$mediaElitePerformanceClass = RelatorioController::corDaLinhaPelaPerformance($fatoFilho->mediaElitePerformance);
				$fatoFilho->mediaElitePerformanceClass = $mediaElitePerformanceClass;
				...
				if(count($todosFilhos)){
					foreach($todosFilhos as $filho){
						...
						$fatoFilho->mediaElitePerformance = ($somaEp/$divisor);
				...
				if(count($todosFilhos)){
					...
					foreach($todosFilhos as $filho){
							...
							if(count($discipulos1)){
								foreach($discipulos1 as $filho1){
										...
										if(count($discipulos2)){
											foreach($discipulos2 as $filho2){
													...
													if(count($discipulos3)){
														foreach($discipulos3 as $filho3){
															...
															if($idEntidadeTipo3 === EntidadeTipo::igreja){
																...
																if((int) $tipoRelatorio === 6){
																	$relatorioPorIgrejaDaRegiao[$filho->getId()][] = $fatoFilho3;
																}
															}
														}
													}
												...
												if($idEntidadeTipo2 === EntidadeTipo::igreja){
													...
													if((int) $tipoRelatorio === 6){
														$relatorioPorIgrejaDaRegiao[$filho->getId()][] = $fatoFilho2;
													}
												}
											}
										}
									...
									if($idEntidadeTipo1 === EntidadeTipo::igreja){
										...
										if((int) $tipoRelatorio === 6){
											$relatorioPorIgrejaDaRegiao[$filho->getId()][] = $fatoFilho1;
										}
									}
								}
							}
						...
						if((int) $tipoRelatorio !== 6){
							...
							$fatoFilho->setMediarealizada($somaRealizada/$diferencaDePeriodos);
							$fatoFilho->setMediarealizadap($somaRealizadap/$diferencaDePeriodos);
							$fatoFilho->mediaElitePerformance = $somaRealizadap/$diferencaDePeriodos;
		...
		if(
			(int) $tipoRelatorio !== 4 && 
			(int) $tipoRelatorio !== 6
		){/* resumo e membresias por igrejas */
			// ordernar os malucos
			...
			foreach($todosFilhos as $filho){
				...
				if((int) $tipoRelatorio === 6){
					$fatoMensal = new FatoMensal();
					$fatoMensal->numero_identificador = -1;
					$fatoMensal->entidade = $grupoFilho->getEntidadeAtiva()->getNome();
					$fatoMensal->lideres = $grupoFilho->getNomeLideresAtivos();
					$relatorio[] = $fatoMensal;
					foreach($relatorioPorIgrejaDaRegiao[$filho->getId()] as $relatorioIgreja){
						$fatoFilho = $relatorioIgreja;
						// ajustando percentual somado
						for($w = 1;$w <= 6;$w++){
							$cqmeta = 'cqmeta' . $w;
							$cbqmeta = 'cbqmeta' . $w;
		...
		$fatoFinal->mediaElitePerformanceClass = 'dark';
	...
	static function fatoMensalSomado($filho, $mes, $ano, $pessoalOuEquipe, $repositorio, $diferencaDePeriodos, $grupo = null){
		...
		$dataInativacao = null;
		if ($filho !== null && $filho->getData_inativacao()) {
			$dataInativacao = $filho->getData_inativacaoStringPadraoBanco();
		}
		$numeroIdentificadorFilho = $repositorio->getFatoCicloORM()->montarNumeroIdentificador($repositorio, $grupoFilho, $dataInativacao);
	...
	public function setentaBispaAction() {
		set_time_limit(0);
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', '120');
		$sessao = new Container(Constantes::$NOME_APLICACAO);
		...
		if($request->isPost()){
			...
			$arrayPeriodoDoMesAtual = Funcoes::encontrarPeriodoDeUmMesPorMesEAno($mes, $ano);
			$diferenca = $arrayPeriodoDoMesAtual[1] - $arrayPeriodoDoMesAtual[0];
			$diferencaDePeriodos = self::diferencaDePeriodos($arrayPeriodoDoMesAtual[0], $arrayPeriodoDoMesAtual[1], $mes, $ano);		
			$diferenca += 1;
			$campoCelula = 'cq'.$diferenca;
			$campoCelulaBeta = 'cbq'.$diferenca;
				...
				foreach ($grupoPaiFilhoFilhos12 as $filho12) {
					...
					if($entidadeDoFilho12->getEntidadeTipo(true)->getId() !== EntidadeTipo::secretario){
						...
						if($entidadeDoFilho12->getEntidadeTipo()->getId() === EntidadeTipo::igreja){
							...
							$relatorioParcial = array();
							foreach ($fatosMensal12 as $fatoMensal12) {
								$tamanhoNumero = strlen($fatoMensal12->getNumero_identificador());
								if($tamanhoNumero === 24){
									$totalDeCelulas = $fatoMensal12->$campoCelula + $fatoMensal12->$campoCelulaBeta;
									$fatoMensal12->setMultiplicadormetasetenta($totalDeCelulas);
									if($fatoMensal12->getMultiplicadormetasetenta() > 0){
										if(
											...
											$parceiro = $this->getRepositorio()->getFatoFinanceiroORM()->fatosValorPorNumeroIdentificadorMesEAno($fatoFilho->getNumero_identificador(), $mes, $ano, $tipoRelatorio = 2)['valor'];					
											$fatoFilho->somaParceiro = $parceiro;

											$relatorioParcial[] = $fatoFilho;
										}
									}
								}
							}
